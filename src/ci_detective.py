...
def classify_failure(log_text: str) -> Diagnosis:
    text = clean_logs(log_text)
    lowered = text.lower()
    tests = extract_failing_tests(text)
    errors = extract_error_lines(text)
    files = extract_traceback_files(text)
            ...
            if not evidence:
                for line in text.splitlines():
                    if needle in line.lower():
                        evidence.append(line.strip()[:500]); break
...
def main() -> int:
    ...
    if not repo or not run_id:
        print("CI Detective: GitHub Actions environment not detected."); return 0
    try:
        ...
        if not failed:
            print("CI Detective: no failed jobs found."); return 0
        diagnoses = []
        ...
        for job in failed:
            ...
            if not diagnosis.history: diagnosis.history = analyze_remote_commit(repo, sha, diagnosis.traceback_files)
