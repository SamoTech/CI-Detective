...
def analyze_git_history(log_text:str)->list[str]:
    ...
    if matching: evidence.append(f"Possible regression correlation: current commit {head[:7]} changed an implicated source file; this is correlation, not proof of causation.")
    return evidence
...
def analyze_remote_commit(repo:str,sha:str,implicated:list[str])->list[str]:
    ...
    if matching:evidence.append(f"Possible regression correlation: failing commit {sha[:7]} changed an implicated source file; this is correlation, not proof of causation.")
    return evidence
...
def render_markdown_report(job_name:str,diagnosis:Diagnosis)->str:
    ...
    if any("Possible regression correlation" in x for x in combined): lines += ["", "### Assessment","**Possible regression correlation:** the failing commit changed a source file referenced by the failure traceback. This is evidence of correlation, not proof that the change caused the failure."]
...
def render_multi_job_report(diagnoses:list[tuple[str,Diagnosis]])->str:
    """Render one report containing every failed job in the workflow run."""
    lines=["## CI Detective — Failure Diagnosis","",f"**Failed jobs:** **{len(diagnoses)}**"]
    for index,(job_name,diagnosis) in enumerate(diagnoses,1):
        lines += ["",f"### Failed Job {index}: `{job_name}`","",f"**Category:** `{diagnosis.category}`  ",f"**Confidence:** **{diagnosis.confidence.upper()}**","", "#### Diagnosis", diagnosis.summary]
        if diagnosis.root_cause and diagnosis.root_cause!=diagnosis.summary: lines += ["", "#### Root Cause", diagnosis.root_cause]
        if diagnosis.implicated_symbol: lines += ["", "#### Implicated Symbol", f"`{diagnosis.implicated_symbol}`"]
        if diagnosis.failing_tests: lines += ["", "#### Failing Tests"]+[f"- `{x}`" for x in diagnosis.failing_tests[:5]]
        if diagnosis.traceback_files: lines += ["", "#### Traceback Files"]+[f"- `{x}`" for x in diagnosis.traceback_files[:5]]
...
def main()->int:
    ...
    try:
        ...
        report=render_multi_job_report(diagnoses); print(report); write_step_summary(report)
        ...
        if output_path:
            payload={"category":diagnoses[0][1].category,"confidence":diagnoses[0][1].confidence,"summary":diagnoses[0][1].summary,"root_cause":diagnoses[0][1].root_cause,"implicated_symbol":diagnoses[0][1].implicated_symbol,"failing_tests":diagnoses[0][1].failing_tests,"traceback_files":diagnoses[0][1].traceback_files,"history":diagnoses[0][1].history}
