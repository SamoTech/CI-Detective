...
def post_or_update_pr_comment(repo: str, pr_number: int, report: str) -> str:
    ...
    if existing:
        gh_api(f"repos/{repo}/issues/comments/{existing['id']}", "PATCH", {"body": body})
        return "updated"
...
def main() -> int:
    ...
    try:
        ...
        if os.environ.get("INPUT_COMMENT_ON_PR", "true").lower() == "true":
            ...
            if pr_number:
                result = post_or_update_pr_comment(repo, pr_number, report)
                print(f"CI Detective: PR comment {result}.")
            else:
                print("CI Detective: no pull request context; skipping PR comment.")
